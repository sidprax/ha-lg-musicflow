...
class LGMusicFlowConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_wizard_menu(self, user_input: dict[str, Any] | None = None) -> FlowResult:
    ...
    async def async_step_wizard_ethernet(self, user_input: dict[str, Any] | None = None) -> FlowResult:
        ...
        if user_input is not None:
            ...
            try:
                ...
                try:
                    ...
                    info = prod.get("info", {})
                    mac = info.get("wirelessmac") or info.get("btmac") or host
                    name = spk_name or info.get("name") or prod.get("petname") or f"LG Speaker ({host})"
                    await self.async_set_unique_id(mac)
                    self._abort_if_unique_id_configured(updates={CONF_HOST: host})
                    return self.async_create_entry(
                        title=name,
        ...
        return self.async_show_form(step_id="wizard_ethernet", data_schema=schema, errors=errors)

    async def async_step_wizard_hotspot_guide(self, user_input: dict[str, Any] | None = None) -> FlowResult:
    ...
    async def async_step_ssdp(self, discovery_info: SsdpServiceInfo) -> FlowResult:
        ...
        try:
            ...
            info = prod.get("info", {})
            mac = info.get("wirelessmac") or info.get("btmac") or host
            self._discovered_name = info.get("name") or prod.get("petname") or f"LG Speaker ({host})"
